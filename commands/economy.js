 ...
 * @modified by : @Mnuvq <https://github.com/Mnuvq>
 ...
 **/
 ...
 cmd({
	...
	const akeno = "akeno"
	const daily  = await eco.daily(citel.sender, akeno, 2000); //give 500 for daily, can be changed
	 if (daily.cd) { //cdL is already formatted cooldown Left
        ...
        return await Void.sendButtonText(citel.chat, buttons, `You already claimed daily for today, come back in ${daily.cdL}`, `Akeno-Economy
Version: 0.0.6`, citel);
	 } else {
...
async(Void, citel, text,{ isCreator }) => {
    ...
    const balance  = await eco.balance(user, akeno)
    switch (value) {
        case '1000':
        case '1':
        if (k > balance.wallet ) return citel.reply(`*_You need to pay 💎100 to increase bank capacity ~ 1000 sp_*`);
          const deduct1 = await eco.deduct(user, akeno, 100);
          const add1 = eco.giveCapacity(user, akeno, 1000);
              let buttons = [{
                buttonId: `${prefix}wallet`,
                buttonText: {
                    displayText: "Wallet👛",
        ...
        return await Void.sendButtonText(citel.chat, buttons, `*1000 💎diamond storage has been added in ${pushname} bank*`, `Akeno-Economy
Version: 0.0.6`, citel);
        ...
        if (k < balance.wallet) return citel.reply(`*You need to pay 💎1000 to increase bank capacity ~ 100000 sp*`);
          const deduct2 = await eco.deduct(user, akeno, 1000);
          const add2 = eco.giveCapacity(user, akeno, 100000);
              let buttonss = [{
                buttonId: `${prefix}wallet`,
                buttonText: {
                    displayText: "Wallet👛",
        ...
        return await Void.sendButtonText(citel.chat, buttonss, `*100000 💎diamond storage has been added in ${citel.pushName} bank*`, `Akeno-Economy
Version: 0.0.6`, citel);
        ...
        if (k < balance.wallet) return citel.reply(`You need to pay 💎10000 to increase bank capacity ~ 1000 sp`);
           const deduct3 = await eco.deduct(user, akeno, 10000);
           const add3 = eco.giveCapacity(user, akeno, 10000000);
               let buttonst = [{
                buttonId: `${prefix}wallet`,
                buttonText: {
                    displayText: "Wallet👛",
        ...
        return await Void.sendButtonText(citel.chat, buttonst, `*10000000 💎diamond storage has been added in ${citel.pushName}\'s bank*`, `Akeno-Economy
Version: 0.0.6`, citel);
 ...
 await citel.reply('*What are you trying to do📉*.')
    ...
    async(Void, citel, text,{ isCreator }) => {
      ...
      //  let users = citel.mentionedJid ? citel.mentionedJid[0] : citel.msg.contextInfo.participant || false;
        ...
        let d = parseInt(text)
        const deposit = await eco.deposit(citel.sender, "akeno", d);
        const balance = await eco.balance(citel.sender, "akeno")
    ...
    return await Void.sendButtonText(citel.chat, buttons, `Successfully 💰Deposited 💎${deposit.amount} to your bank.Upgrade your bank capacity to add more money📈.`, `Akeno-Economy
Version: 0.0.6`, citel);
    }
)
...
async(Void, citel, text,{ isCreator }) => {
    ...
    const akeno = "akeno"
        ...
        const balance = await eco.balance(user1, akeno);
        let a = (balance.wallet) < parseInt(word)
        //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
        if(a == true) return citel.reply("you dont have sufficient money to transfer👎");

        const deduct = await eco.deduct(user1, akeno, value[0]);
        const give = await eco.give(user2, akeno, value[0]);
        let buttons = [{
            buttonId: `${prefix}wallet`,
            buttonText: {
                displayText: "Wallet👛",
    ...
    return await Void.sendButtonText(citel.chat, buttons, `*📠 Transaction successful of ${value[0]} 💰*`, `Akeno-Economy
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
         const akeno = "akeno"
         const balance = await eco.balance(citel.sender, akeno); //Returns wallet, bank, and bankCapacity. Also creates a USer if it doesn't exist.
         let buttons = [{
...
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        if(!isCreator) return
         ...
         await eco.give(users, akeno, parseInt(text.split(' ')[0]));
...
Version: 0.0.6`, citel);
	...
	const akeno = "akeno"
	    const k = 250
        const balance1  = await eco.balance(user1, akeno)
	const balance2  = await eco.balance(user2, akeno)
    ...
    switch (random) {
        case 'ran':
              ...
              break
        case 'rob':
          const deduct1 = await eco.deduct(user2, akeno, balance2.wallet);
          const add2 = eco.give(user1, akeno, balance2.wallet);
              await citel.reply(`*🤑 Robbery operation successfully.🗡️*`)

              break
        case 'caught':
           const deduct2 = await eco.deduct(user1, akeno, balance1.wallet);
...
default:
 ...
 ///.mentionedJid[0] ? m.mentionedJid[0] : m.sender
     ...
     if (!text) return citel.reply(
        ...
        if ( r == opp){
           let give = await eco.give(user , akeno, twice);
           let buttons = [{
            buttonId: `${prefix}slot`,
            buttonText: {
                displayText: "Slot🎰",
    ...
    return await Void.sendButtonText(citel.chat, buttons, `*📈 You won 💎${twice}*`, `Akeno-Economy
Version: 0.0.6`, citel);

        }
        else{
           let deduct = await eco.deduct(user, akeno, texts[0]);
           let buttons = [{
            buttonId: `${prefix}slot`,
            buttonText: {
                displayText: "Slot🎰",
    ...
    return await Void.sendButtonText(citel.chat, buttons,`*📉 You lost 💎${texts[0]}*`, `Akeno-Economy
Version: 0.0.6`, citel);
    ...
    async(Void, citel, text,{ isCreator }) => {
        ...
        if (today.getDay() == 6 || today.getDay() == 5 || today.getDay() == 0){
            ...
            const akeno = "akeno"
            const k = 100
            const balance1  = await eco.balance(user, akeno)
...
if(value<=balance.wallet){
    ...
    if ((f1 !== f2) && f2 !== f3){
        const deduct1 = await eco.deduct(user, akeno, deduff);
        return citel.reply(`${mess1}\n\n*Big Lose -->* _💎${deduff}_`)
     }
     else if ((f1 == f2) && f2 == f3){
        const give1 = await eco.give(user, akeno, deduff/2);
        return citel.reply(`${mess2}\n*_Little Jackpot -->* _💎${deduff/2}_`)
     }
     else if ((f1 == f2) && f2 !== f3){
        const give2 = await eco.give(user, akeno, deduff);
        return citel.reply(`${mess3}\n*Small Win -->* _💎${deduff}_`)
     }
     else if ((f1 !== f2) && f1 == f3){
        const deduct2 = await eco.deduct(user, akeno, deduff);
        return citel.reply(`${mess5}\n\n*Small Lose -->* _💎${deduff}_`)
     }
     else if ((f1 !== f2) && f2 == f3){
        const give4 = eco.give(user, akeno, deduff);
        return citel.reply(`${mess3}\n\n*Small Win -->* _💎${deduff}_`)
     }
     else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
        const give5 = eco.give(user, akeno, deduff*20);
    ...
    return citel.reply('You don\'t have enough 💰amount in your👛 wallet.\n- Please don\'t provide 🤑amount.')
            ...
            if ((f1 !== f2) && f2 !== f3){
               const deduct1 = await eco.deduct(user, akeno, 50);
                      citel.reply(`${mess1}\n\n*Big Lose -->* _💎50_`)
            }
            else if ((f1 == f2) && f2 == f3){
               const give1 = await eco.give(user, akeno, 100);
                     citel.reply(`${mess2}\n*_Little Jackpot -->* _💎100_`)
            }
            else if ((f1 == f2) && f2 !== f3){
               const give2 = await eco.give(user, akeno, 20);
                     citel.reply(`${mess3}\n*Small Win -->* _💎20_`)
            }
            else if ((f1 !== f2) && f1 == f3){
               const deduct2 = await eco.deduct(user, akeno, 20);
                     citel.reply(`${mess5}\n\n*Small Lose -->* _💎20_`)
            }
            else if ((f1 !== f2) && f2 == f3){
               const give4 = eco.give(user, akeno, 20);
                     citel.reply(`${mess3}\n\n*Small Win -->* _💎20_`)
            }
            else if ((f1 == f2) && (f2 == f3) && (f3 == f4)){
               const give5 = eco.give(user, akeno, 1000);
                    citel.reply(`${mess4}\n\n_🎊 JackPot --> _💎1000_`)
            }
            else {
...
async(Void, citel, text,{ isCreator }) => {
    ...
    if (mongoschemas == "false") return citel.reply("*🚦Economy* is not active in current group.");
    const kg = 100
            const balance1  = await eco.balance(citel.sender, "akeno")
    ...
    if(lol){
        ...
        const give2 = await eco.give(citel.sender, "akeno", deduff*10);
    ...
    return await Void.sendButtonText(citel.chat, buttons,str+`You got ${deduff*10} in your wallet.`, `Akeno-Economy
Version: 0.0.6`, citel);

    } else {
    const deduff = Math.floor(Math.random() * 300)
    const deduct1 = await eco.deduct(citel.sender, "akeno", deduff);
    let st = `\n🎰 Slot Machine Result\n     ${i}\n\n      ${j}\n\n      ${k}\n\nNot Jacpot📉 but lost `
            let str = st.replace(/1/g, `🔴`).replace(/2/g, `🔵`).replace(/3/g, `🟣`).replace(/4/g, `🟢`).replace(/5/g, `🟡`).replace(/6/g, `⚪️`).replace(/7/g, `⚫️`).replace(/:/g, `    `)
    let buttons = [{
        buttonId: `${prefix}slot`,
...
return await Void.sendButtonText(citel.chat, buttons,str+` ${deduff}.`, `Akeno-Economy
Version: 0.0.6`, citel);
}
}
)
